...
contract FlightSuretyApp {
    ...
    SDFlightSuretyData flightSuretyData;
    ...
    */
                                ...
                                public 
    {
        ...
        flightSuretyData = SDFlightSuretyData(dataContractAddress);
        _registerAirline("Shailesh Airlines", contractOwner);
    }
...
abstract contract SDFlightSuretyData {

    function isOperational() public view virtual returns(bool);

    function isAirlineRegistered(address airline) external view virtual returns(bool);

    function isAirlineFunded(address airline) external view virtual returns(bool);

    function isAirlinePending(address airline) external view virtual returns(bool);

    function getRegisteredAirlineCounter() public view virtual returns(uint256);

    function registerAirline(string memory name, address airlineAddress) external virtual;

    function addPendingAirline(string memory name, address airlineAddress) external virtual;

    function voteForAirline(address airlineAddress) external virtual returns (uint256);

    function fundAirline(address airlineAddress, uint256 amount) external payable virtual; 

    function buy(bytes32 flightKey, address passengerAddress, uint256 insuredAmount) external payable virtual;

    function creditInsurees(bytes32 flightKey) external virtual;

    function pay(address passengerAddress) external payable virtual;

    function getPassengerCredit(address passangerAddress) external view virtual returns (uint256);
}